...
fn example_offsetting() -> Result <(), Box<dyn std::error::Error>> {
	...
	let points: Vec<(f64, f64)> = vec![(0., 0.), (100., 0.), (40., 20.), (100., 20.), (100., 60.), (0., 60.), (0.0, 0.0)];
	// The size of our margin offset, if this value is egal to 0, no offsetting will be computed

	// The size of our margin offset
	let offset_size: f64 = 4.2;
	// Tolerance is the Arc Segments precision in polygon offsetting (rounded corner)
	let tolerance: f64 = 0.1; // 

	let mut polygon: Polygon = Polygon::new(&points, offset_size).map_err(|e| { e })?;
    ...
    draw_svg_offset(
        ...
        "example_offsetting1.svg".to_string(),
	).map_err(|e| { 
